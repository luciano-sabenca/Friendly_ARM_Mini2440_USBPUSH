...
int main(int argc, char **argv)
{
	...
	if(usb_get_driver_np(hdl,0,drivername,30) == 0){
		printf("Driver %s attached for board!\nFor precausion detaching driver!\n", drivername);
                printf("Detaching kernel driver! Result is %d\n", usb_detach_kernel_driver_np(hdl,0));
	} else {
		printf("Could not find attached driver for board! Doing nothing!\n");
	}
